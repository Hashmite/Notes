...
1.  使用函数Hook，使用 useRoutes(数组)，返回一个变量 ，将该变量放在需要展示的位置即可渲染视图
...
1. ```tsx
   在react-router-dom v6中，虽然基于<Route> 匹配渲染，但是不会基于属性将 
     history、location、match传递给组件，想获取相关的信息，我们只能基于Hook 函数处理
   ...
   在react-router-dom v6中，实现路由跳转的方式：
     ...
     2.<Navigate to="/a" /> 遇到这个组件就会跳转（至重定向）
